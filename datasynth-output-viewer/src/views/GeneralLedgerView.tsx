...
import { loadJournalEntriesCsv, loadChartOfAccounts, getAccountNameByAncestor } from '../api/data';
...
export function GeneralLedgerView() {
  ...
  const selectedAccountName = selectedAccount ? getAccountNameByAncestor(selectedAccount, coaMap) : null;
      ...
      <div className="general-ledger-selector">
        ...
        >
          {accountList.map((acct) => {
            const name = getAccountNameByAncestor(acct, coaMap);
            const label = name !== acct ? `${acct} — ${name}` : acct;
            return (
        ...
        {selectedAccount && (
          <span className="general-ledger-account-name" title={selectedAccountName ?? selectedAccount}>
            {selectedAccountName ?? selectedAccount}
          </span>
        )}
        {coaMap.size === 0 && (
          <span className="general-ledger-coa-hint" title="chart_of_accounts.json not found or empty — names are account codes">
            (no COA)
          </span>
        )}
