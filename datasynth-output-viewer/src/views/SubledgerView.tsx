...
function arColumns(lang: SubledgerLang, _showFec: boolean): Col[] {
  const L = lang === 'fr';
  const base: Col[] = [
              ...
              <div className="subledger-accordion-header">
                <span className="subledger-accordion-title">
                  {(() => {
                    ...
                    const row = selectedRow as Record<string, unknown>;
                    if (activeTab === 'ar') {
                      return String(row.invoice_number ?? row.document_id ?? '');
                    }
                    if (activeTab === 'ap') {
                      return String(row.invoice_number ?? row.vendor_invoice_number ?? '');
                    }
                    if (activeTab === 'fa') {
                      return String(row.asset_number ?? '');
                    }
