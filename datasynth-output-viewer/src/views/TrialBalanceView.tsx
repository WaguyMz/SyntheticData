...
import { loadJournalEntriesCsv, loadChartOfAccounts, getAccountInfoByAncestor } from '../api/data';
...
function computeTrialBalance(
  ...
  for (const [account_code, agg] of byAccount.entries()) {
    ...
    const { name: account_name, category } = getAccountInfoByAncestor(account_code, coaMap);
    entries.push({
      account_code,
      account_name,
      category,
