...
const FEC_SEARCH_KEYS: readonly (keyof FECRow)[] = [
  "Code journal",
  "Libellé journal",
...
export function JECView() {
  ...
  const filteredJeData = useMemo(() => {
    ...
    return jeData.filter((row) =>
      matchesSearch(row as Record<string, unknown>, search, JE_SEARCH_KEYS as readonly (keyof Record<string, unknown>)[])
    );
  }, [jeData, search]);
  ...
  const filteredFecData = useMemo(() => {
    ...
    return fecData.filter((row) =>
      matchesSearch(row as Record<string, unknown>, search, FEC_SEARCH_KEYS as readonly (keyof Record<string, unknown>)[])
    );
  }, [fecData, search]);
