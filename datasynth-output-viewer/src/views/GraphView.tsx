...
export function GraphView({ resolvedTheme = 'dark' }: { resolvedTheme?: 'light' | 'dark' }) {
  ...
  const [onlyAuxAccountsConnectedComponents, setOnlyAuxAccountsConnectedComponents] = useState(true);
  ...
  const displayData = useMemo(() => {
    if (rawJeData) {
      return buildGraphFromJournalEntries(rawJeData.rows, rawJeData.coaNames, {
        ...
        onlyAuxAccountsConnectedComponents: onlyAuxAccountsConnectedComponents,
  ...
  }, [rawJeData, graphData, whitelistSet, onlyAuxAccountsConnectedComponents, accountMaxDepth]);
            ...
            <>
              ...
              <div className="graph-auxiliary-wrap">
                <label>
                  <input
                    type="checkbox"
                    checked={onlyAuxAccountsConnectedComponents}
                    onChange={(e) => setOnlyAuxAccountsConnectedComponents(e.target.checked)}
                    aria-label="Only auxiliary accounts connected components"
                  />
                  <span>Only auxiliary accounts connected components</span>
                </label>
                <p className="graph-auxiliary-hint">
                  When on (default): show only the connected component(s) that include auxiliary accounts (e.g. 401xxxx, 411xxxx). When off: full GL graph without auxiliary nodes.
                </p>
              </div>
            </>
          )}
