...
export interface GraphBuildOptions {
  ...
  /** When true (default), add auxiliary nodes and show only connected components that include at least one auxiliary account. When false, full GL graph without auxiliary nodes. */
  onlyAuxAccountsConnectedComponents?: boolean;
...
export const DEFAULT_ACCOUNT_MAX_DEPTH = 4;
...
/** 408 (GR/IR, factures non parvenues) is shown as a normal GL node so it stays visible in the graph. */
function is408Gl(gl: string): boolean {
  const t = gl.trim();
  return t === '408' || t === '408000' || t.startsWith('408');
}

/** French PCG: fournisseurs (401…) and clients (411…). Not 400 (sales), not 408 (GR/IR). When includeAuxiliary we skip these as endpoints so we get shortcut edges (other ↔ aux) only. */
function isVendorOrCustomerGl(gl: string): boolean {
  if (is408Gl(gl)) return false;
  if (gl.startsWith('41')) return true;
  if (gl.length >= 3 && gl.startsWith('40') && gl.charAt(2) !== '0') return true;
  return false;
}

/** True if this node id is a fournisseurs/clients aggregate to exclude when includeAuxiliary (shortcut: no 40/41 nodes). 408 is kept visible. */
function isVendorOrCustomerNode(id: string): boolean {
  if (is408Gl(id)) return false;
 ...
 */
/**
 * Compute connected components (undirected). Returns a map nodeId -> componentId.
 */
function connectedComponents(nodes: { id: string }[], links: { source: string; target: string }[]): Map<string, string> {
  ...
  const visit = (id: string) => {
    if (idToComp.has(id)) return;
    const c = `c${compId}`;
    idToComp.set(id, c);
    for (const n of adj.get(id) ?? []) visit(n);
  };
  for (const n of nodes) {
    if (!idToComp.has(n.id)) {
      visit(n.id);
      compId += 1;
    }
  }
...
export function buildGraphFromJournalEntries(
  ...
  const onlyAuxCC = options?.onlyAuxAccountsConnectedComponents !== false; // default true
  const includeAuxiliary = onlyAuxCC; // when onlyAuxCC we need aux nodes to filter by component
  ...
  /** Resolve name by first COA ancestor: retract digits from right to left until a match. */
  function coaNameForTruncated(truncatedCode: string): string {
    const code = (truncatedCode ?? '').trim();
    if (!code) return code;
    for (let len = code.length; len >= 1; len--) {
      const prefix = code.slice(0, len);
      const name = coaNames.get(prefix);
      if (name) return name;
    }
    return code;
  }
  ...
  // When onlyAuxAccountsConnectedComponents: keep only connected components that contain at least one auxiliary node
  if (onlyAuxCC && nodes.length > 0) {
    const nodeIdToComp = connectedComponents(nodes, links);
    const compHasAux = new Set<string>();
    for (const n of nodes) {
      const id = String(n.id);
      if (id.startsWith(AUX_PREFIX)) {
        const c = nodeIdToComp.get(id);
        if (c != null) compHasAux.add(c);
      }
    }
    // If no auxiliary nodes in data, show full graph (no filtering)
    if (compHasAux.size > 0) {
      const keepNodeIds = new Set<string>();
      for (const n of nodes) {
        const c = nodeIdToComp.get(n.id);
        if (c != null && compHasAux.has(c)) keepNodeIds.add(n.id);
      }
      const filteredNodes = nodes.filter((n) => keepNodeIds.has(n.id));
      const keepLinks = links.filter(
        (l) => keepNodeIds.has(String(l.source)) && keepNodeIds.has(String(l.target))
      );
      return { nodes: filteredNodes, links: keepLinks };
    }
  }
